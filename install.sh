#!/bin/bash
###############################################################################
# Copyright (c) 2011-2014 libbitcoin-server developers (see COPYING).
#
...
build_all()
{
    ...
    build_from_travis libbitcoin-server libbitcoin-server protocol $PARALLEL "$@" $BITCOIN_SERVER_OPTIONS
}
